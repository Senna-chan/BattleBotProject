...
namespace Firmata.NET
{
    ...
    class Arduino
    {
        ...
        private const int STRING_SEND = 0x71; // Sends string
        ...
        public void Open()
        {
            ...
            sendLcdCommand("KEVIN IS EEN FGT");
        }
        ...
        public void sendLcdCommand(string lines)
        {
            var data = str_to_two_byte_iter(lines + "\0");
            var str = new StringBuilder();
            str.Append(START_SYSEX);
            str.Append(STRING_SEND);
            str.Append("LCD:");
            foreach(byte bt in data)
            {
                var ch = Convert.ToChar(bt);
                str.Append(ch);
            }
            str.Append(END_SYSEX);
            _serialPort.Write(str.ToString());
        }
        ...
        private byte[] str_to_two_byte_iter(string str)
        {
            var bstring = str;
            var bytes = new byte[str.Length];
            var i = 0;
            foreach(var ch in bstring)
            {
